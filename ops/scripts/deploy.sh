...
if [[ "${SKIP_GIT_SYNC}" != "1" ]]; then
  ...
  echo "[deploy] skip_git_sync=1 (workspace snapshot mode)"
  DEPLOYED_SHA="$(printf '%s' "${DEPLOY_SHA_OVERRIDE}" | tr -d '[:space:]')"
  if [[ -z "${DEPLOYED_SHA}" ]]; then
    if [[ -d .git ]] && run_as_app git rev-parse --verify --quiet HEAD >/dev/null 2>&1; then
      DEPLOYED_SHA="$(run_as_app git rev-parse HEAD)"
    else
      DEPLOYED_SHA="snapshot-$(date -u +%Y%m%d%H%M%S)"
    fi
  fi
fi
...
if [[ "${SKIP_DEPLOY_SELFTEST}" == "1" ]]; then
  echo "[deploy] skipping npm run selftest (HLAUTO_DEPLOY_SKIP_SELFTEST=1)"
else
