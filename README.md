...
- AskQuestion は頻度制御付き:
  ...
  - 推奨閾値（初期値）:
    ...
    - `ASKQUESTION_TRIGGER_POSITION_NOTIONAL_RATIO=0.8`（`RISK_MAX_POSITION_NOTIONAL_USD` の80%）
    - `ASKQUESTION_TRIGGER_RECONCILE_FAILURE_STREAK=2`
    - `ASKQUESTION_TRIGGER_WS_TIMEOUTS_15M=2`
    - `ASKQUESTION_TRIGGER_BLOCKED_AGE_MS=1800000` + `ASKQUESTION_TRIGGER_BLOCKED_GROWTH_15M=50`
...
BOT_DECISION_V2
questionId=
action=HOLD|RESUME|PAUSE|FLATTEN|CANCEL_ORDERS|CUSTOM|REJECT
ttl_sec=
reason=
```
...
- Runtime safety:
  ...
  - `ASKQUESTION_SUPPRESS_FLAT_LOW_RISK`
...
- GitHub Actions の `deploy-vps` は deploy 後に `--summary-only` で過去24hサマリを自動出力
  ...
  - repoスナップショット適用を先に試し、失敗時は `git fetch/checkout` をbest-effortで実施して service restart
  - 追加: deploy直後に `10 minutes ago` の quick check を実行し、Invariant A/B は warning として可視化
